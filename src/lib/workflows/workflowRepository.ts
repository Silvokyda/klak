...
export function validateWorkflowSteps(steps: WorkflowStep[]): void {
  const allowed = new Set(["open_url", "open_folder", "launch_app", "run_command_template", "start_background_process", "create_note", "copy_to_clipboard", "search_memory", "create_memory", "manual_instruction"]);
  for (const step of steps) {
...
function inferWorkflowRisk(steps: WorkflowStep[]): RiskLevel {
  if (steps.some((step) => ["run_command_template", "start_background_process"].includes(step.type))) return "high";
